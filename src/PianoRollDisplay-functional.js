import { generateGradientTable } from './helpers/generateGradientTable.js';


function PianoRollSvg(props) {

  // PianoRoll brand #5DB5D5
  const backgroundStartColor = { r: 93, g: 181, b: 213 };
  // #154151
  const backgroundEndColor = { r: 21, g: 65, b: 81 };
  backgroundColormap = generateGradientTable(backgroundStartColor, backgroundEndColor, 128);

  const noteStartColor = { r: 66, g: 66, b: 61 };
  const noteEndColor = { r: 28, g: 28, b: 26 };
  noteColormap = generateGradientTable(noteStartColor, noteEndColor, 128);

  //rysuje sekwencję 60 dźwięków
  function drawPianoRoll (sequence) {
    start = sequence[0].start
    end = sequence[sequence.length - 1].end - start
  ...
  function drawEmptyPianoRoll(pitch_min, pitch_max) {
  ...
  //Czy muszę przerobić cały ten kod?
